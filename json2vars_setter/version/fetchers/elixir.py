...
# Stable Elixir tags look exactly like "v1.18.4"; release candidates
# ("v1.20.0-rc.0") and the moving "vX.Y-latest" tags do not match this pattern
# and are therefore excluded.
_STABLE_TAG_PATTERN = re.compile(r"v\d+\.\d+\.\d+$")


class ElixirVersionFetcher(BaseVersionFetcher):
    """Fetches Elixir version information from GitHub"""

    def __init__(self) -> None:
        """Initialize with Elixir's GitHub repository information"""
        super().__init__("elixir-lang", "elixir")

    def _is_stable_tag(self, tag: JsonObject) -> bool:
        """
        Check if a tag represents a stable Elixir release
    ...
    def _parse_version_from_tag(self, tag: JsonObject) -> ReleaseInfo:
        ...
        # Clean version string (e.g., "v1.18.4" -> "1.18.4")
    ...
    def _get_stability_criteria(
        ...
        """
        Determine the stable and latest versions of Elixir
...
def elixir_filter_func(tag: JsonObject) -> bool:
    """Filter function for Elixir tags in API checker"""
    name = str(tag.get("name", ""))
    return bool(_STABLE_TAG_PATTERN.fullmatch(name))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch Elixir version information")
    parser.add_argument(
    ...
    if args.verbose > 0:
        ...
        check_github_api(
            session=session,
            github_owner="elixir-lang",
            github_repo="elixir",
            count=args.count,
            filter_func=elixir_filter_func,
        )

    # Display header in verbose mode
    if args.verbose > 0:
        print("\n=== Fetching Elixir Versions ===")

    # Main processing
    fetcher = ElixirVersionFetcher()
