...
# Stable Julia tags look exactly like "v1.11.2"; pre-releases ("v1.13.0-rc1",
# "v1.13.0-beta3", "v1.13.0-alpha1") carry a suffix and are therefore excluded.
_STABLE_TAG_PATTERN = re.compile(r"v\d+\.\d+\.\d+$")


class JuliaVersionFetcher(BaseVersionFetcher):
    """Fetches Julia version information from GitHub"""

    def __init__(self) -> None:
        """Initialize with Julia's GitHub repository information"""
        super().__init__("JuliaLang", "julia")

    def _is_stable_tag(self, tag: JsonObject) -> bool:
        """
        Check if a tag represents a stable Julia release
    ...
    def _parse_version_from_tag(self, tag: JsonObject) -> ReleaseInfo:
        ...
        # Clean version string (e.g., "v1.11.2" -> "1.11.2")
    ...
    def _get_github_tags(self, count: Optional[int] = None) -> List[JsonObject]:
        ...
        The JuliaLang/julia tag API is not reliably newest-first, so fetch a
        generous set of stable tags and sort them by semantic version before
        truncating; this guarantees ``_get_stability_criteria`` sees the real
        latest release rather than whatever the API happened to list first.
        """
    ...
    def _get_stability_criteria(
        ...
        """
        Determine the stable and latest versions of Julia
...
def julia_filter_func(tag: JsonObject) -> bool:
    """Filter function for Julia tags in API checker"""
    name = str(tag.get("name", ""))
    return bool(_STABLE_TAG_PATTERN.fullmatch(name))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch Julia version information")
    parser.add_argument(
    ...
    if args.verbose > 0:
        ...
        check_github_api(
            session=session,
            github_owner="JuliaLang",
            github_repo="julia",
            count=args.count,
            filter_func=julia_filter_func,
        )

    # Display header in verbose mode
    if args.verbose > 0:
        print("\n=== Fetching Julia Versions ===")

    # Main processing
    fetcher = JuliaVersionFetcher()
