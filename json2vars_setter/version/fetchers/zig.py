...
# Stable Zig tags are plain "X.Y.Z" (e.g. "0.15.2"); development/nightly builds
# are published as "master" or "X.Y.Z-dev.*" and do not match this pattern.
_STABLE_TAG_PATTERN = re.compile(r"\d+\.\d+\.\d+$")


class ZigVersionFetcher(BaseVersionFetcher):
    """Fetches Zig version information from GitHub"""

    def __init__(self) -> None:
        """Initialize with Zig's GitHub repository information"""
        super().__init__("ziglang", "zig")

    def _is_stable_tag(self, tag: JsonObject) -> bool:
        """
        Check if a tag represents a stable Zig release
    ...
    def _parse_version_from_tag(self, tag: JsonObject) -> ReleaseInfo:
        ...
        # Zig stable tags are already a bare version (e.g. "0.15.2")
        version = name
    ...
    def _get_stability_criteria(
        ...
        """
        Determine the stable and latest versions of Zig

        - latest: the most recent release version
        - stable: usually the previous minor version of latest (Zig is still
          pre-1.0, so "minor" is the second version component)
...
def zig_filter_func(tag: JsonObject) -> bool:
    """Filter function for Zig tags in API checker"""
    name = str(tag.get("name", ""))
    return bool(_STABLE_TAG_PATTERN.fullmatch(name))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch Zig version information")
    parser.add_argument(
    ...
    if args.verbose > 0:
        ...
        check_github_api(
            session=session,
            github_owner="ziglang",
            github_repo="zig",
            count=args.count,
            filter_func=zig_filter_func,
        )
